...
function touch { 
    param (
        ...
        ".xlsx" {
            # Excel file: Create a valid .xlsx file
            try {
                $Excel = New-Object -ComObject Excel.Application
                    $Workbook = $Excel.Workbooks.Add()

                    # Ensure the path includes the correct extension
                    if (-not $FilePath.EndsWith(".xlsx")) {
                        $FilePath += ".xlsx"
                    }

                # Save the workbook explicitly in .xlsx format
                ...
                # Ensure the Excel process is terminated
                if ($Excel) {
                    $Excel.Quit()
                        [System.Runtime.Interopservices.Marshal]::ReleaseComObject($Excel) | Out-Null
                }
